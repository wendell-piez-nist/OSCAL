...
sure it conforms to the profile?)
  ...
  <xsl:template match="prop" mode="oscal-type">property</xsl:template>
  <xsl:template match="part" mode="oscal-type">part</xsl:template>
  <xsl:template match="p" mode="oscal-type">line</xsl:template>

  <xsl:template match="*" mode="oscal-type">
    <xsl:message terminate="yes">Boo</xsl:message>
  </xsl:template>
  ...
  <xsl:template match="p[matches(@class,'\S')] | prop[matches(@class,'\S')] | part[matches(@class,'\S')]">
    <xsl:element name="{@class}" namespace="http://csrc.nist.gov/ns/oscal/1.0">
      ...
      <xsl:attribute name="type">
        <xsl:text>oscal:</xsl:text>
        <xsl:apply-templates mode="oscal-type" select="."/>
      </xsl:attribute>
